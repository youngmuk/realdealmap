    ...
    return DraggableScrollableSheet(
      ...
      builder: (context, controller) => ListView(
        ...
        children: [
          ...
          // 건물 외곽선 (D-4).
          //
          // 평면도(도면)는 여전히 없다 — 국토부 실거래가에 없고 건축HUB는 별도
          // 활용신청이 선행이다(D-1~D-3). 대신 **건물이 땅 위에 차지한 자리**를
          // 그린다. 맞닿은 집들 사이에서 어느 건물의 거래인지는 그것으로 가려진다.
          //
          // 지도 렌더러는 띄우지 않는다. 하나 더 올리면 그래픽 메모리가 두 배가
          // 되고(지도 하나가 139 MB · 실측), OSM 타일을 직접 받는 길은
          // **이용정책이 배포 앱의 트래픽을 허용하지 않는다.** 폴리곤 몇 개를
          // 캔버스에 그리는 데는 둘 다 필요 없다.
          //
          // 자료가 없으면(지번이 가려진 거래·아직 안 올린 지역) 이 블록은
          // 스스로 사라진다.
          BuildingOutlineBlock(tx: tx),
          //
